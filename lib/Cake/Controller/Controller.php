...
App::uses('AllowedIps', 'Admin.Model');
 ...
 */
	public function invokeAction(CakeRequest $request) {
        ...
        //if IP for logged-in user has changed (outside allowed range) during the session, force log out
        if ($uid) {

            $userLog = new UserLog();

            if (!$allowedIps->checkIp()) {
                //logging action
                $userLog->saveAction($uid, 'logout', 'forced (IP changed)', $_SERVER['REMOTE_ADDR']);
                //force logout user
                $this->Session->destroy();
                return $this->redirect(['login']);
            }
            
            $options['conditions'] = ['User.id' => $uid];
            $userTable = new User();
            $user = $userTable->find('first', $options);
            
            if ($user && ($user['User']['tfa_code_installed'] != 1) && (($request->params['plugin'] != 'user') || $request->params['controller'] != 'profile')) {
                $this->redirect(['plugin' => 'user', 'controller' => 'profile', 'action' => 'index']);
            }
            
        }
        
        //user is not logged in and if IP is out of allowed range
        if (!$allowedIps->checkIp()) {
            $this->autoRender = false;
            $this->layout = 'clear';
